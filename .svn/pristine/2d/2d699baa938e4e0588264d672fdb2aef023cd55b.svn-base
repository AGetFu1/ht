...
{
    public class Calendarzz1Controller : ControllerBase
    {
        ...
        {
            var data = calendarApp.getZZ1ListJson();
        ...
        {
            //var data = calendarApp.getListz1Json(pagination,  queryJson);
            var data = new
            {
                rows = calendarApp.getListzz1Json(pagination, queryJson),//数据
        ...
        {
            calendarApp.Submitzz1Form(calendarEntity, keyValue);
            return Success("操作成功。");
        }
        [HttpGet]
        [HandlerAjaxOnly]
        ...
        {
            return calendarApp.inWeekzz1Model(calendarEntity);
        }
        [HttpPost]
        [HandlerAjaxOnly]
        public ActionResult Scheduling(CalendarEntity calendarEntity, string keyValue)
        {
            string val = calendarEntity.F_Team;
            ...
            if (calendarEntity.F_Title == "周内值班")
            {
                ...
                List<string> arrayds = calendarApp.inWeekzz1Model(calendarEntity);//获取周内排班人员存入数组
                ...
                for (int i = 0; i < workArray.Count; i++)//遍历workArray数组
                {
                    for (int j = n; j < (workArray.Count + arrayds.Count); j++)
                    {
                        ...
                        calendarApp.Submitzz1Form(calendarEntity, keyValue); n++;
                        break;
                    }
                }
            ...
            else if (calendarEntity.F_Title == "周末值班")
            {
                ...
                List<string> arrayds = calendarApp.outWeekzz1Model(calendarEntity);//获取周末排班人员存入数组
                int n = arrayds.IndexOf(calendarEntity.F_Text);//获取首日值班人员索引号
                for (int i = 0; i < workArray.Count; i++)//遍历workArray数组
                {
                    for (int j = n; j < (workArray.Count + arrayds.Count); j++)
