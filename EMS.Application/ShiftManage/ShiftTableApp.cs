...
namespace EMS.Application.ShiftManage
{
    public class ShiftTableApp
    {
        ...
        //=======================================================测试二部================================================================================
        public List<ShiftTableEntity> getListJsoncs2(Pagination pagination, string queryJson)
        {
            var data = service.IQueryable(t => t.F_Department == "测试二部").OrderBy(t => t.F_Team).ToList();
            var queryParam = queryJson.ToJObject();
            if (!queryParam["keyword"].IsEmpty())
            {
                string keyword = queryParam["keyword"].ToString();
                data = service.IQueryable(t => t.F_Department == "测试二部" && (t.F_Name.Contains(keyword) || t.F_Phone.Contains(keyword) || t.F_Team.Contains(keyword))).OrderByDescending(t => t.F_Team).ToList();
            }
            return data;
        }
        public void SubmitFormcs2(ShiftTableEntity shiftTableEntity, string keyValue)
        {
            shiftTableEntity.F_Department = "测试二部";
            if (shiftTableEntity != null)
            {
                switch (shiftTableEntity.F_Team)
                {
                    case "生产组":
                        shiftTableEntity.F_Backgroudcolor = "#FC6805";
                        break;
                    case "质量组":
                        shiftTableEntity.F_Backgroudcolor = "#909399";
                        break;
                    case "测编工程":                       
                        shiftTableEntity.F_Backgroudcolor = "#65B017";
                        break;
                    case "平移工程":
                        shiftTableEntity.F_Backgroudcolor = "#EC6091";
                        break;
                    case "后段工程":
                        shiftTableEntity.F_Backgroudcolor = "#FFB62B";
                        break;
                    default:
                        shiftTableEntity.F_Backgroudcolor = "#FF1493";
                        break;
        ...
        public List<ShiftTableEntity> getTeamJsoncs2(string val)
        {
            List<ShiftTableEntity> data = new List<ShiftTableEntity>();
            if (!string.IsNullOrEmpty(val))
            {
                data = service.IQueryable(t => t.F_Department == "测试二部" && t.F_Team == val).OrderBy(t => t.F_Team).ToList();
            }
            else
            {
                var expression = ExtLinq.True<ShiftTableEntity>();
                data = service.IQueryable(t => t.F_Department == "测试二部").OrderBy(t => t.F_CreatorTime).ToList();
            }
